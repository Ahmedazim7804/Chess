...
class MainWindow(qtw.QMainWindow):
    ...
    icons = {
        "bishop_black": os.path.join("Icons", "bishop_black.svg"),
        "bishop_white": os.path.join("Icons", "bishop_white.svg"),
        "king_black": os.path.join("Icons", "king_black.svg"),
        "king_white": os.path.join("Icons", "king_white.svg"),
        "knight_black": os.path.join("Icons", "knight_black.svg"),
        "knight_white": os.path.join("Icons", "knight_white.svg"),
        "pawn_black": os.path.join("Icons", "pawn_black.svg"),
        "pawn_white": os.path.join("Icons", "pawn_white.svg"),
        "queen_black": os.path.join("Icons", "queen_black.svg"),
        "queen_white": os.path.join("Icons", "queen_white.svg"),
        "rook_black": os.path.join("Icons", "rook_black.svg"),
        "rook_white": os.path.join("Icons", "rook_white.svg"),
        "dot": os.path.join("Icons", "dot.png"),
    }
    ...
    def __init__(self):
        ...
        self.dict_index = (
            {}
        )  # Dictionary of Location of button in 2D array,row and column in tuple
        self.func = {
            "pawn": self.pawn_move,
            "king": self.king_move,
            "rock": self.rock_move,
        ...
        self.turn = True
        self.pieces = {
            1: [j for i in [6, 7] for j in self.lst[i]],
            0: [j for i in [0, 1] for j in self.lst[i]],
        }
        self.castle_dict = {}
        self.undo_var = []
        # Status Bar
        ...
        other.addAction(
            "Help",
            lambda: webbrowser.open("https://www.chess.com/learn-how-to-play-chess"),
        )
        other.addAction(
            "About",
    ...
    def design_board(self):
        ...
        # Rock
        self.dict[self.lst[0][0]] = {
            "piece": "rock",
            "color": 0,
            "move": 0,
            "icon": self.icons["rook_black"],
            "index": (0, 0),
        }
        self.dict_index[(0, 0)] = {
            "piece": "rock",
        ...
        self.dict[self.lst[7][0]] = {
            "piece": "rock",
            "color": 1,
            "move": 0,
            "icon": self.icons["rook_white"],
            "index": (7, 0),
        }
        self.dict_index[(7, 0)] = {
            "piece": "rock",
        ...
        self.dict[self.lst[7][1]] = {
            "piece": "knight",
            "color": 1,
            "icon": self.icons["knight_white"],
            "index": (7, 1),
        }
        self.dict_index[(7, 1)] = {
            "piece": "knight",
        ...
        # Bishop
        self.dict[self.lst[0][2]] = {
            "piece": "bishop",
            "color": 0,
            "icon": self.icons["bishop_black"],
            "index": (0, 2),
        }
        self.dict_index[(0, 2)] = {
            "piece": "bishop",
        ...
        self.dict[self.lst[7][2]] = {
            "piece": "bishop",
            "color": 1,
            "icon": self.icons["bishop_white"],
            "index": (7, 2),
        }
        self.dict_index[(7, 2)] = {
            "piece": "bishop",
        ...
        # King
        self.dict[self.lst[0][4]] = {
            "piece": "king",
            "color": 0,
            "move": 0,
            "icon": self.icons["king_black"],
            "index": (0, 4),
        }
        self.dict_index[(0, 4)] = {
            "piece": "king",
        ...
        self.dict[self.lst[7][4]] = {
            "piece": "king",
            "color": 1,
            "move": 0,
            "icon": self.icons["king_white"],
            "index": (7, 4),
        }
        self.dict_index[(7, 4)] = {
            "piece": "king",
        ...
        # Queen
        self.dict[self.lst[0][3]] = {
            "piece": "queen",
            "color": 0,
            "icon": self.icons["queen_black"],
            "index": (0, 3),
        }
        self.dict_index[(0, 3)] = {
            "piece": "queen",
        ...
        self.dict[self.lst[7][3]] = {
            "piece": "queen",
            "color": 1,
            "icon": self.icons["queen_white"],
            "index": (7, 3),
        }
        self.dict_index[(7, 3)] = {
            "piece": "queen",
    ...
    def ready(self, piece):
        ...
        # ------------For Castling---------------------
        if (
            self.dict[self.pre_sender]["piece"] == "king"
            and self.dict[self.pre_sender]["move"] == 0
        ):
            for r in [0, 7]:
                for c, d in [(3, 2), (5, 6)]:
    ...
    def process(self, box, piece):
        ...
        if self.dict[box]["piece"] == "pawn":
            ...
            self.dict_index[self.dict[box]["index"]]["move"] = (
                self.dict[box]["move"] + 1
            )
        ...
        if a == "change":
            for i in self.valid_moves:
                if self.dict[i]["piece"]:
                    ...
                    i.setIcon(qtg.QIcon(self.icons["dot"]))
        elif a == "reset":
            for i in self.valid_moves:
    ...
    def change_dict(self, piece, box):
        ...
        # -----------For Castling-----------------
        if (
            self.dict[piece]["piece"] == "king"
            and box in self.castle_dict
            and self.dict[piece]["move"] == 0
        ):
        ...
        if self.dict[piece]["piece"] == "pawn":
            ...
            self.dict_index[box_index]["first_move"] = self.dict_index[piece_index][
                "first_move"
            ]
    ...
    def pawn_move(self, piece, r, c):
        ...
        try:
            if (
                self.dict_index[(r + 1 * sign, c + 1)]["piece"]
                and self.dict_index[r + 1 * sign, c + 1]["color"]
                ...
                try:
                    for i in range(1, 8):
                        if self.dict_index[r + i * r_sign, c + i * c_sign]["piece"]:
                            if (
                                self.dict_index[r, c]["color"]
                                != self.dict_index[r + i * r_sign, c + i * c_sign][
                                    "color"
                                ]
                            ):
                                moves.append(
                ...
                for i, j in [(1, 2), (2, 1)]:
                    try:
                        if self.dict_index[r + i * r_sign, c + j * c_sign]["piece"]:
                            if (
                                self.dict_index[r, c]["color"]
                                != self.dict_index[r + i * r_sign, c + j * c_sign][
                                    "color"
                                ]
                            ):
                                moves.append(
                ...
                if self.dict_index[r, c]["piece"] == "rock":
                    if self.dict_index[r, c]["move"] == 0:
                        ...
                        if self.dict_index[r, i]["button"] in lst:
                            self.castle_dict[self.dict_index[r, d]["button"]] = (
                                r,
                                c,
                                i,
                            )
    ...
    def before_moves(self, piece, valid_moves):
        ...
        for i in valid_moves:
            ...
            if name == "king":
                for j in a:
                    if (
                        self.dict[j]["piece"] == "king"
                        and self.dict[j]["color"] == color
                    ):
                        king_only.append(i)
            # ----------------------------
        ...
        for i in all_moves:
            if self.dict[i]["piece"] == "king" and self.dict[i]["color"] != color:
                qtw.QMessageBox.about(
                    self, "", f"Check To {self.statusBar_dict[not(color)]}"
                )
    ...
    def checkmate(self, color, arg):
        ...
        if len(all_moves) == 0:
            if arg == "checkmate":
                qtw.QMessageBox.about(
                    self, "", f"Checkmate To {self.statusBar_dict[color]}"
                )
                return "checkmate"
            else:
    ...
    def promotion(self, piece):
        ...
        queen = qtw.QPushButton("Queen")
        queen.clicked.connect(lambda: self.promotion_2(piece, "queen"))
        rock = qtw.QPushButton("Rock")
        rock.clicked.connect(lambda: self.promotion_2(piece, "rock"))
        knight = qtw.QPushButton("Knight")
        knight.clicked.connect(lambda: self.promotion_2(piece, "knight"))
        bishop = qtw.QPushButton("Bishop")
        bishop.clicked.connect(lambda: self.promotion_2(piece, "bishop"))
